...
def append_gap_entry(path: Path, entry_text: str) -> None:
    """Append `entry_text` to `gaps.md`, ensuring a separating newline before it."""
...
def remove_gap_entry(path: Path, gap_id: str) -> ParsedGap | None:
    """Remove the entry with the given id from `gaps.md`.

    Returns the removed `ParsedGap` (with its span) on success, or `None`
    if no entry with that id is present (idempotent — caller's no-op).
    """
    if not path.exists():
        return None
